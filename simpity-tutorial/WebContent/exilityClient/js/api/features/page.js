...
var ExilityPage = function (win, name)
{
    this.win = win;
    ...
    this.idSplitter = "___"; // id of a table element = "eleId<idSplitter><uniqueRowNum>"
    this.listServices = null;
    this.globalFields = new Array(); //accumulates fields that are to take value from a global field
    this.globalFieldLabels = new Array(); //accumulates fields whose labels are to be taken from globallabels
    this.reportServices = null;
    this.trackFieldChanges = false;
    this.fieldNames = []; //keeps unqualified field names in the order that are added
...
//We start with methods on P2. Start with simple add/set/get methods to dip into the collections
...
ExilityPage.prototype.getParameter = function (name)
{
    return this.pageParameters[name];
};
...
ExilityPage.prototype.addField = function (field)
{
    ...
    if (field.listServiceId)
    {
        ...
        //gridName = serviceId. Except when keyValeu is specified in which case it is serviceId + '_' keyValue
        gridName = field.listServiceId;
        if (field.keyValue) gridName += '_' + field.keyValue;
...
ExilityPage.prototype.getField = function (name)
{
    return this.fields[name];
};

ExilityPage.prototype.getFieldObject = function (name, idx)
{
    var field = this.fields[name];
    if (!field)
        return null;
    var id = field.table ? field.table.getObjectId(name, idx) : name;
...
ExilityPage.prototype.getAction = function (name)
{
...
ExilityPage.prototype.addTable = function (table)
{
    ...
    //A service is not associated with any table. Client does not know what data is being asked when a server call is made.
    //It is a crude design to send pginationSize for all tables. To facilitate this, we accumulate all of them in an array of arrays. See next function addPageSizes()
    ...
    //small optimization to trigger linked/merged table utilities that has to be triggered after all tables are loaded.
    if (table.linkedTableName || table.mergeWithTableName || table.nestedTableName)
...
ExilityPage.prototype.getTable = function (name)
{
    return this.tables[name];
};

//getFiled value will return the value in the field. Note that field value is always synchronized with
// the dom element.
// index is optional, and is used by the field.getValue if it happens to be in a table, in which case the field value from that row is returned
ExilityPage.prototype.getFieldValue = function (name, idx, toFormat, fromDeletedRowAsWell)
{
    var field = this.fields[name];
    if (!field)
    { //try local variable
        debug(name + ' is not a field. Trying local variable');
        if(this.win.name_value_)
        {
	        field = this.win.name_value_[name];
        ...
        debug(name + ' is not found any where ');
        return null;
    }
...
ExilityPage.prototype.hideField = function (name)
{
    this.hideOrShowField(name, 'none');
};
ExilityPage.prototype.hideOrShowField = function (name, display)
{
    var field = this.fields[name];
    if (field && field.table)
    {
        field.table.hideOrShowField(name, display);
        return;
    }
    var ele = this.doc.getElementById(name);
    if (ele)
    {
        ele.style.display = display;
        ele = this.doc.getElementById(name + 'Label');
        if (ele)
            ele.style.display = display;
        ele = this.doc.getElementById(name + 'Picker');
        if (ele)
            ele.style.display = display;
        ele = this.doc.getElementById(name + 'Operator');
        if (ele)
            ele.style.display = display;

        //is this a date element with time ?
        ele = this.doc.getElementById(name + 'FieldsTable');
        if (ele)
            ele.style.display = display;

        ele = this.doc.getElementById(name + 'ToDiv');
        if (ele)
            ele.style.display = display;
    }

    var eleLeft = this.doc.getElementById(name + 'Left');
    if (eleLeft)
    {
    ...
    var eleMiddle = this.doc.getElementById(name + 'Middle');
    if (eleMiddle)
    {
    ...
    var eleRight = this.doc.getElementById(name + 'Right');
    if (eleRight)
    {
    ...
    else if (field && field.validationStatusOnHide) //Sep 08 2009 : Bug 477 - Buttons are not hiding when Hidden is given - Exility: Venkat  
...
ExilityPage.prototype.setDefaultValue = function (fieldName, value)
{
    ...
    else debug('field ' + fieldName + ' is not a feild and hence a default value of ' + value + ' is not set');
};
//index is optional. It is used only if the field is inside a table. For a table, if this is omitted,
// current selected row is assumed.
//returns true if the value is assigned to a field, else false for the fact that it got assigned to a window variable
ExilityPage.prototype.setFieldValue = function (name, value, obj, idx, suppressDesc, formatValue, listServiceOnLoad, pageParameterOnLoad) // Aug 06 2009 :  Bug ID 631 - suppressDescOnLoad is not working - WeaveIT: Venkat 
{
    if (!name)
    {
    ...
    if (!field)
    { //try local variable
        debug(name + ' is not a field. value is not set');
        if (!this.win.name_value_)
           this.win.name_value_ = {};
        this.win.name_value_[name] = value;
        //this.win[name] = value;
        return false;
    }
    if (pageParameterOnLoad)
        field.setValue(value, obj, idx, field.supressDescOnLoad, formatValue, null, listServiceOnLoad); // Aug 06 2009 :  Bug ID 631 - suppressDescOnLoad is not working - WeaveIT: Venkat
    else
        field.setValue(value, obj, idx, suppressDesc, formatValue, null, listServiceOnLoad); // Aug 06 2009 :  Bug ID 631 - suppressDescOnLoad is not working - WeaveIT: Venkat
};

ExilityPage.prototype.setFieldLabel = function (name, label)
{
    var f = this.doc.getElementById(name + 'Label');
    if (!f) return;
...
{
    //in case some pagination is pending, we will try to tell server that we are done
    try
    {
        for (var tableName in this.tables)
        {
            var table = this.tables[tableName];
            if (table.totalRows && table.totalRows > table.nbrRows) table.requestPage(0);
        }
    }
    catch (e) { }
};
...
{
    // We have been inserting some variable into a newly opened window in the window that opened it. for e.g.
    // win=window.open(blaaaaa); win.someValriable = 'someValue'. This is not reliable across browsers,
    // probably because of the time taken to create the window object, and when it is avialable..
    // To solve this, I am introducing the ocncept of an inbox for a window. Note that the caller should create the inbox
    // before creating the window
    getInbox(this.win);
    //this inbox is not destroyed now. It will be used again if this page is to be reset. It is upto the caller
    // to change the inbox..
    ...
    if (this.listServices)
    {
        ...
        for (var a in this.listServices)
        {
            listeners = this.listServices[a];
            aRow = [listeners.serviceId, listeners.keyValue ? listeners.keyValue : ''];
            toPush = false;
            for (var i = 0; i < listeners.length; i++)
            {
                ...
                if (!field.obj)
                    debug('DESIGN ERROR: ' + field.name + ' could not get its dom element at the time of regestering for list service');
                if (field.noAutoLoad)
                    continue;
    ...
    if (this.reportServices)
    {
        ...
        for (var a in this.reportServices)
        {
            listeners = this.reportServices[a];
            aRow = [listeners.serviceId, listeners.keyValue ? listeners.keyValue : ''];
    ...
    if (this.buttonsToEnable)
    {
        for (var i = 0; i < this.buttonsToEnable.length; i++)
            this.buttonsToEnable[i] = this.doc.getElementById(this.buttonsToEnable[i]);
    }
    if (this.buttonsToDisable)
    {
        for (var i = 0; i < this.buttonsToDisable.length; i++)
            this.buttonsToDisable[i] = this.doc.getElementById(this.buttonsToDisable[i]);
    }
    ...
    if (exilParms.errorAtBottom) //do we show field errors in a panel that is always at the bottom?
        this.initErrorPanel();
    ///////// we are done with formatting the page. We are ready to receive data now. /////
    ...
    for (var p in this.pageParameters)
    {
        ...
        if ('pageheader' == param.name && param.value)
        {
            ...
            this.setFieldValue((param.setTo ? param.setTo : p), param.value, null, null, null, null, null, true); // Sep 23 2009 : Bug 697 - In The Modify Mode 'Description serviece is Calling' - SABMiller : Aravinda
    }
    ...
    if (grid.length > 1)
        this.callListService(grid, null, null, null, null, null, true); // Aug 06 2009 :  Bug ID 631 - suppressDescOnLoad is not working - WeaveIT: Venkat
    ...
    if (actions && actions.length > 0)
    {
        ...
        for (var i = 0; i < n; i++)
            this.act(this.doc.body, 'body', actions[i], null, true); // Aug 06 2009 :  Bug ID 631 - suppressDescOnLoad is not working - WeaveIT: Venkat
    }
    else
        this.showPage();
    ...
    if (this.hotkeyFunction)
    {
        if (this.doc.addEventListener)
            this.doc.addEventListener("keydown", this.win[this.hotkeyFunction], false);
        else
            this.doc.onkeydown = this.win[this.hotkeyFunction];
    }
    else if (exilParms.appHotKeyFunctionName && exilParms.appHotKeyFunctionName != '')
    {
        if (this.doc.addEventListener)
            this.doc.addEventListener("keydown", PM[exilParms.appHotKeyFunctionName], false);
        else
            this.doc.onkeydown = function () { 
        		var ret = PM[exilParms.appHotKeyFunctionName](this.win.event.keyCode); 
        		if (!ret) 
...
ExilityPage.prototype.addDatePlaceHolders = function ()
{
	...
	{
		var ele = eles[i];
		if(ele.className == 'dateinputfield' && !ele.getAttribute('placeholder'))
...
ExilityPage.prototype.showPage = function ()
{
    ...
    var lft = 0;
    var frameEle = null;
    if (this.win.name == "pickerWindow" || this.win.isAPopup) //it is popup mode
    {
        ...
        if (this.win.name == "pickerWindow")
        {
            ...
            pickerEleStyle.display = ''; // Feb 22 2011 : Bug 2123 - code picker should not show blank screen - Exility App : Aravinda
            // Dec 21 2010 : Bug 1893 - The pop-up page is not positioned properly - Exility (End) : Aravinda
        }
        else
        {
            var parentDiv = this.win.parent && this.win.parent.document.getElementById(this.win.name + 'Div');
            if (parentDiv)
            {
                parentDiv.style.display = ''; // Feb 25 2011 : Fixed for Popup - Exility App : Aravinda
    ...
    //Let the page show-up now. Note: Idealy it shoudl show-up after all onload services return
...
ExilityPage.prototype.processPageParameters = function ()
{
    ...
    for (p in params)
    {
        ...
        if (!val)
        {
            ...
            if (param.isRequired)
            {
                message('Internal error: ' + p + ' is a required parameter for this page. It is not passed by the caller.', "Error", "Ok", null, null, null);
            }
            continue;
        }
        if (param.dataType)
        {
            ...
            catch (e)
            {
                ...
                message('Internal error: This page is called with ' + p + ' = ' + val + '. This is not a valid value.' + e.messageText, "Error", "Ok", null, null, null);
                continue;
            }
        }
    ...
    {
        // Oct 28 2008 : Incorporating Changes from Tanmay : Aravinda
        message('This page is to be invoked with ' + this.minParameters + ' parameters. Only ' + n + 'parameters are passed. This is a set-up problem. This page may not work properly', "Error", "Ok", null, null, null);
        return null;
    }
    ...
    for (var i = 0; i < fieldNames.length; i++)
    {
        ...
        if (!field && table)//enable/disable table click action
        {
            if (table.enableAction)
            {
                ...
                debug('Error: ' + id + ' is a table but it does not have enableAction() method. enable/disable action ignored');
            continue;
        }
        ...
        //this needs to be delegated to table to take care of more complex tables (like frozen columns etc..)
        var inputObj = null;
        if (currRow)
        {
        ...
        if (disabled)
        {
            ...
            if ((ele.tagName && ele.tagName.toUpperCase() == 'FONT') || (ele.type && ele.type.toUpperCase() == 'BUTTON')) // Aug 7 2009 :  Bug ID 632 -  Value is Undefined - Exis: Vijay 
            {
                ele.style.cursor = '';
        ...
        else
        {
            if (ele.savedClassName)
                ele.className = (ele.savedClassName == 'disabledinput') ? '' : ele.savedClassName || '';

            if ((ele.tagName && ele.tagName.toUpperCase() == 'FONT') || (ele.type && ele.type.toUpperCase() == 'BUTTON'))   
            {
        ...
        if (ele.childNodes.length > 0)
        {
            for (var j = 0; j < ele.childNodes.length; j++)
            {
                ...
                {
                    if ((ele.childNodes[j].name).search(/Radio/) > 0 || (ele.childNodes[j].name).search(/Checkbox/) > 0)
                    {
        ...
        //too many loops and too many operations. isn't inputObj same as field.table.htmlRowToClone.getElementsByTagName(nodeName)? And, why is that done inside the loop??
        if (rowObjArr.length)
        {
            for (var j = 0; j < rowObjArr.length; j++)
            {
                for (var k = 0; k < inputObj.length; k++)
                {
                    if (rowObjArr[j].id == inputObj[k].id && inputObj[k].getAttribute('disabled') == false)
                    {
                        field.table.htmlRowToClone.getElementsByTagName(nodeName)[k].removeAttribute('disabled', 0);
                    }
                }
            }
        }
        ...
        // Nov 26 2009 : Bug 796 - calender control-problem with hidding the datefield - Exility (End) : Venkat 
    }
};

// Called to get values for a drop-downreturns a dc populated with list serviceId and keyvalue pairs
ExilityPage.prototype.callListService = function (grid, field, fieldName, qryFields, qrySources, idx, listServiceOnLoad) // Aug 06 2009 :  Bug ID 631 - suppressDescOnLoad is not working - WeaveIT: Venkat
{
    var dc = new DataCollection();
    dc.addGrid('listServiceIds', grid);
    if (qryFields)
    {
        if (!this.fillDcFromQueryFields(dc, qryFields, qrySources, idx)) return false;
    }
    var se = new ServiceEntry('listService', dc, false, listServiceReturned, field, fieldName, ServerAction.NONE, null, null, null, listServiceOnLoad); // Aug 06 2009 :  Bug ID 631 - suppressDescOnLoad is not working - WeaveIT: Venkat
    this.win.serverStub.callService(se);
};

// Called to get values for a report returns a dc populated with report serviceId and keyvalue pairs
ExilityPage.prototype.callReportService = function (grid, field, fieldName, qryFields, qrySources, idx)
{
    var dc = new DataCollection();
    ...
    var se = new ServiceEntry('reportService', dc, false, reportServiceReturned, field, fieldName, ServerAction.NONE);
    this.win.serverStub.callService(se);
};

// Expecting the value to fillDcFromQueryFields for repeating columns.
ExilityPage.prototype.fillDcFromQueryFields = function (dc, names, sources, idx, toValidate, fieldVal, fieldName)
{
    if (!names) return true;
    if (!sources)
    {
        message("Design error: Source not supplied for name = " + names, "Error", "Ok", null, null, null);
        return false;
    }
    var n = names.length;
    var val;
    ...
    for (var i = 0; i < n; i++)
    {
        ...
        if (field)
        {
            if (toValidate == "true" && field.name != fieldName) // Nov 21 2009 : Bug 767 - ValidateQueryFields="false" - Exility: Venkat
            {
...
ExilityPage.prototype.fillDcFromForm = function (dc, fields, tables, sendAllFields)
{
    if (!fields)
    {
        fields = this.fields;
    ...
    for (fieldName in fields)
    {
        ...
        else
        {
            ...
            if (grid)
                ...
                debug(fieldName + ' is neither a field, nor a javascript variable name. It will not be submitted.');
        }
    }
...
// supplied, should have exactly same number of comma separated fields as the naems
ExilityPage.prototype.makeQueryString = function (names, sources, idx, toValidate, pickerSource)
{
    ...
    {
        // Oct 28 2008 : Incorporating Changes from Tanmay : Aravinda
        message('Design error: Sources not supplied for name ' + names, "Error", "Ok", null, null, null);
        return;
    }
    var val;
    ...
    for (var i = 0; i < sources.length; i++)
    {
        ...
        {
            val = field.getValue(idx);
            if (toValidate == "true" && (!pickerSource || field.name != pickerSource)) // Nov 21 2009 : Bug 767 - ValidateQueryFields="false" - Exility: Venkat
            {
                try
                {
                    val = field.validateValue(val);
...
ExilityPage.prototype.resetLastModifiedNode = function ()
{
    ...
    //is there an error area that is rendering existing errors? Errors will all be obsolete now
 ...
 * @lends ExilityPage
 * @param {Object} event - The onclick event from table header th.
 * @param {String} tableName - The name of the table.
 * @param {columnName} columnName -The name of the column in table.
 * @author vinay 03-August-2013
 * */
ExilityPage.prototype.tableHeaderClicked = function (event, tableName, columnName)
{
...
var pageActionActAfterMessageOk = function ()
{
    ...
    p2BeingConfirmed.act(actionBeingConfirmed.obj, actionBeingConfirmed.fieldName, actionBeingConfirmed.actionName, actionBeingConfirmed.param, actionBeingConfirmed.actionOnLoad);
...
ExilityPage.prototype.act = function (obj, fieldName, actionName, param, actionOnLoad)
{
    debug('P2.act() with actionName = ' + actionName + ' obj=' + obj + ' fieldName=' + fieldName);
    var msgDiv = document.getElementById('warningMsg'); //used by exis team
    if (msgDiv)
        msgDiv.style.display = 'none';

    if (obj && obj.type && obj.getAttribute('eleDisabled') == 'true') // Jul 22 2009 :  Bug ID 611 - Java Script Error - India Services: Venkat
        return;
    ...
    {
        if ((exilParms.trackFieldChanges == true) || (exilParms.trackFieldChanges == false && this.trackFieldChanges == true))
        {
            if (this.lastModifiedNode)
            {
                ...
                PM.message('Do you want to abandon change(s)?', 'Warning', 'Yes', pageActionActAfterMessageOk, 'No', pageActionActAfterMessageNotOk, null, null);
                return;
            }
        }
    }
    ...
    if (action.popupPanel)
        ...
        this.popDown();
    var actionDone = action.act(obj, fieldName, param, actionOnLoad); // Aug 06 2009 :  Bug ID 631 - suppressDescOnLoad is not working - WeaveIT: Venkat
    //if it is not a serverAction, and if we are to show page at end.
    if (action.showPage && !action.serviceId)
        this.showPage();
    //moved reset to the end to handle the case when a service did not fire because of validation issues.
...
ExilityPage.prototype.hideOrShowPanels = function (panelNames, display)
{
    ...
    for (i = 0; i < panelNames.length; i++)
    {
        ...
        if (!panel)
        {
            debug("design error: panelName " + panelName + " is to be hidden/shown, but it is not a dom element");
            continue;
        }
        // is it tabbed panel?
        ...
        if (tab)
        {
            ...
            {
                tab.style.display = '';
                // May 20 2009 :  Bug 482 - Issue while hiding Tab   - ADCP (Start) : Venkat
                if (tabLeft)
                    tabLeft.style.display = '';
                if (tabRight)
                    tabRight.style.display = '';
                // May 20 2009 :  Bug 482 - Issue while hiding Tab   - ADCP (End) : Venkat
                tab.onclick.call(tab);
                continue;
            }
            // we have to hide. If this is the active tab, let us make another tab active
            if (tab.clickDisabled || tab.getAttribute('clickdisabled'))
            {
                var newtab = tabLeft.previousSibling;
                if (newtab)
                    newtab = newtab.previousSibling;
                //keep going back till you get a tab that is not hidden, or there is no more
                while (newtab)
    ...
    //page size would have increased. If animated, then animationEnd() would take care of this
    //if (!exilParms.animateHideAndShow)
    this.win.checkIframeSize();
};

//show a panel as if it is a pop-up. We whould be able to move it, but as of now, it is a simple popup
//We use a simple tehnique.  cover the page. Jack-up the panel by making its posiiton 'relative' and a hihger zIndex.
ExilityPage.prototype.popupAPanel = function (panelName)
{
    //panel may have a top to include borders etc.. get the top, failing which get this panel
    var panel = this.doc.getElementById(panelName + 'Top') || this.doc.getElementById(panelName);
    if (!panel)
    {
        debug('Error: ' + panelName + ' is not found as a DOM element. popup option failed ');
        return;
    }
...
//when panels are animated, we don't know when to resize window(to avoid double-scroll-bar)
...
ExilityPage.prototype.appendRows = function (tableName, data)
{
    ...
    if (table)
    {
        ...
        debug('Table ' + tableName + ' not found for this page. Rows will not be appended');
};

// push values from dc to page
ExilityPage.prototype.refreshPage = function (dc, listServiceOnLoad) // Aug 06 2009 :  Bug ID 631 - suppressDescOnLoad is not working - WeaveIT: Venkat
{
    var table, field;

    // Apr 30 2009 :  Bug 457 - Selection Field is selecting first value even though other value exists  - Exis (Start) : Venkat
    this.fillList(dc.grids, true); // Aug 06 2009 :  Bug ID 631 - suppressDescOnLoad is not working - WeaveIT: Venkat
    this.fillReport(dc.grids);
    // Apr 30 2009 :  Bug 457 - Selection Field is selecting first value even though other value exists  - Exis (End) : Venkat

    for (var a in dc.values) //setfieldvalue(fieldname, value, obj, index, supressDesc, toFormat
    {
        var supressDesc = false;
        if (this.fields[a] && this.fields[a].supressDescOnLoad)
            supressDesc = this.fields[a].supressDescOnLoad;
        this.setFieldValue(a, dc.values[a], null, 0, supressDesc, true, listServiceOnLoad); //last true is for formatting.. // Aug 06 2009 :  Bug ID 631 - suppressDescOnLoad is not working - WeaveIT: Venkat
    }
    ...
    for (var a in dc.grids)
    {
        ...
        if (table)
        {
            //some tables do not want data to be set to them directly. Typically linked tables.
            if (table.doNotSetDataToMe)
                continue;
            table.setData(dc, null, null, listServiceOnLoad); // Aug 06 2009 :  Bug ID 631 - suppressDescOnLoad is not working - WeaveIT: Venkat
        }
...
ExilityPage.prototype.resetFields = function (fieldsToReset)
{
    ...
    for (var i = 0; i < n; i++)
    {
        ...
        if (field && field.toBeSentToServer && field.value != field.defaultValue)
...
/**
 * reset fields to be submitted for a server action. 
 * This feature provides flexibility for designer to re-use the same server action 
 * with different sets of data. Primarility designed for auto-save feature for UXD project
 * @param actionName {String} name of service
 * @param fields {String[]} fields to be sent to server
 */
...
ExilityPage.prototype.validate = function (fields, tables)
{
    ...
    //as of now, when we find an error, we alert the messsage, and hence it makes sense to
    // take the user to the field in error right away. That means, we stop validation. 
    // However, it is possible that we can validate all fields in one go, and accumulate all erros on the way.
    // messgaes can be rendered in such a way that user can click on any of them, and go to the field in error.
    // In such a design, just set the next variable to false, and change e.render()
    // 
    ...
    for (var fieldName in fields)
    {
        ...
        if (!field || !field.toBeSentToServer)
            continue;
        var onlyIfChanged = this.validateOnlyOnUserChange || field.validateOnlyOnUserChange;
    ...
    for (var tableName in tables)
    {
        ...
        catch (e)
        {
            if (!e.messageId) //it is not an exility error
            {
                ...
                debug(' script error in ' + fName + ' line no ' + e.lineNo + ' ' + e.source + '\n' + e.message);
                return false;
            }
            if (exilParms.stopAtFirstError)
            {
                PM.pageValidation = false; // Nov 30 2009 : Bug 758 - Filter field-Between value Issue - Exis (Start): Aravinda
                return false;
            }
            returnValue = false;
        }
    ...
    if (returnValue && userFunction)
    {
        ...
        {
            debug("user function " + userFunction + " has aborted with an error." + e);
            returnValue = false;
        }
...
//Have you read the comments on the top about event qing issues? Do that before trying to understand this method
ExilityPage.prototype.fieldChanged = function (obj, fieldName, internalChange, supressDesc, listServiceOnLoad) // Oct 20 2009 : Bug ID 737 - suppressDescOnLoad='true' for dependant selection field is triggering description service on reload. - WeaveIT: Venkat
{
    ...
    {
        debug('field ' + fieldName + ' is Delete. Hence ignoring the field change');
        isDeleteField = true;
    }
    ...
    {
        if (!isDeleteField)
            debug('designError: field ' + fieldName + ' not found in fields collection');
        return;
    }
    ...
    if (table)
    {
        ...
        if (table.isRowDeleted && table.isRowDeleted(idx))
        {
            ...
            field.setValueToObject(obj, val);
            message("You cannot modify a deleted row. Please un-delete the row before modifying it.", "Warning", "Ok", null, null, null);
            return false;
        }
    }
    ...
    // a field attached with desc service is not considered to be changed until either the code picker returns or the
    ...
    else if (!internalChange) //validate only of user entered the value
    {
        try
        {
            ...
            if (field.validationFunction)
            {
                ...
                if (typeof (fn) == 'function')
                {
                    ...
                    if (errorText && errorText.length)//user defined funciton has returned an error
        ...
        catch (e)
        {
            ...
            if (field.triggerFieldChangedEvents)
                field.triggerFieldChangedEvents('', obj, idx, supressDesc, internalChange);
            return;
        }
    }
    ...
    if (table && table.rowChanged)
        table.rowChanged(obj, field.unqualifiedName, idx, internalChange);
    //RGB:22-Dec-08. If code picker is pending, we should set .isValid to false. Set it to true for other cases
    //obj.isValid = true;
    if (pp && pp.obj == obj)
    {
        obj.isValid = false;
    ...
    if (field.triggerFieldChangedEvents)
        field.triggerFieldChangedEvents(formattedValue, obj, idx, supressDesc, internalChange, listServiceOnLoad);
};

ExilityPage.prototype.catchError = function (e)
{
    //signal to other scripts that may run that we haev just raised aan error.
    //500 ms is acceptable, because anyways we will be showing an error message that the user has to see..
...
ExilityPage.prototype.keyPressedOnField = function (obj, objName, e)
{
    ...
    // it is not possible to determine based on metadata, because of cloneRow and multi-panel grids
...
ExilityPage.prototype.createCodePicker = function (pickerObj, e, fieldName)
{
    ...
    var x = e.clientX;
    if (this.left) x = x + this.left;
    var y = e.clientY;
    if (this.top) y = y + this.top;
...
ExilityPage.prototype.startCodePicker = function ()
{
    ...
    {
        var qry = this.makeQueryString(field.descQueryFields, field.descQueryFieldSources, idx, field.validateQueryFields, field.name);
        if (qry == false)
            return false;
        src += '?' + qry;
    }
...
ExilityPage.prototype.codePickerReturned = function (data)
{
    ...
    PM.codePickerLeft = -1;
    PM.codePickerTop = -1;
    //if user didn't change the field, nor did she choose any code from picker, move on...
    ...
    else //user had modified the code, opened the picker and then closed it woithout a selection
...
ExilityPage.prototype.createDatePicker = function (pickerObj, e, fieldName)
{
    ...
    obj = this.doc.getElementById(id);
    var xy = getPageXy(obj); //defined in utils. gets x,y relative to window top taking care of scrolls
    //position date picker 
    var pTop = xy.y + obj.offsetHeight; //just below this object
    var pLeft = xy.x + Math.floor((obj.offsetWidth - 190) / 2); //and center aligned
...
ExilityPage.prototype.startDatePicker = function ()
{
    ...
    if (val)
    {
        if (dt && dt.includesTime)
            ...
            val = parseDate(val); //parseDate will return null if it is not a valid date. That is fine with us
    }
    var minDate = null;
    var maxDate = null;
    if (dt && exilParms.restrictDatesInCalendar)
    {
    ...
    if (calTop + 185 > window.innerHeight) //not enough space below, move it above
    {
        calTop = calTop - 200;
        if (calTop < 0) //not enough space above as well!!! it will overlap on the date field now.
            calTop = 10;
    }
    var xy = getPageXy(calElement.offsetParent); //these are the coordinates of calendar's parent relative to window. this + top/jeft should give rise to pp.x/y
    calElement.style.top = (calTop - xy.y) + "px";
    calElement.style.left = (pp.x - xy.x) + "px";
    coverPage(coverCalendarStyle, imgCalendarStyle);
    //debug('for calendar objLeft:' + pp.x + ' objTop:' + calTop + ' calLeft:' + xy.x + ' calTop' + xy.y);
...
ExilityPage.prototype.datePickerReturned = function (date)
{
    ...
    var pp = this.pendingPicker;
    this.pendingPicker = null;
    //if the field was not changed and the user did not choose he date, we have no work
...
// Jan 07 2010 : Bug 844 - Issue related to ValidateQueryFields attribute - WeaveIT (Start): Venkat
...
ExilityPage.prototype.showErrorInPanel = function (obj, text)
{
    ...
    //debug('Trying to show error text ' + text + ' for field ' + (obj && obj.id));
    var ele;
    var id = null;
    ...
    {
        id = obj.id;
        var lbl = this.doc.getElementById(id.split('__')[0] + 'Label');//in case obj is inside a table
        if (lbl)
...
/*
* Remove error message related to a field.
* obj : dom object that represents the field that was in error. If this is not supplied (e.g. error that is not related to a field) ele must be supplied
* retainPanel : optional. If set to true, container panel is not removed even if this was the last error message
* ele: error element that needs to be removed. Used when the error panel may not be associated with a field (e.g. error from server)
*/
ExilityPage.prototype.removeErrorFromPanel = function (obj, retainPanel, ele)
{
    ...
    if (!ele.parentNode)
    {
        debug('Invalid elements supplied to removeErrorFromPanel(). obj = ' + obj + '=obj ele=' + ele);
        return;
    }
...
{
    this.addRowTimer = null; //it could have been triggered with a timer that was set when tab was pressed
    var table = this.tables[tableName];
    ...
    // Dec 09 2009 :  Bug ID 798 -  In single selction value-inside grid   - Exility(Start): Vijay 
    if ((table.dataForNewRowToBeClonedFromRow && table.dataForNewRowToBeClonedFromRow != 'none') || table.dataForNewRowToBeClonedFromFirstRow || table.rowsCanBeCloned) // Feb 16 2010 :  Bug ID 967 - Drop down issue in Grid - Exility : Venkat 
        table.cloneFlag = 1;
    else
        table.cloneFlag = 0;
    // Dec 09 2009 :  Bug ID 798 -  In single selction value-inside grid   - Exility(End) : Vijay 
    ...
    if (f)
    {
        try
        {
            if (f.indexOf('(') >= 0) //use has typed a funciton with parameters
                eval('this.win.' + f);
            else //it is name of a function
                this.win[f]();
        } catch (e)
        {
            debug(f + ' could not be run as a function. An exception occurred: ' + e);
        }
    }
};
...
ExilityPage.prototype.cloneTableRow = function (obj, tableName)
{
    ...
    if (this.tables[tableName].dataForNewRowToBeClonedFromFirstRow || this.tables[tableName].rowsCanBeCloned)
...
ExilityPage.prototype.removeTableRow = function (e, obj, tableName)
{
...
ExilityPage.prototype.deleteTableRow = function (e, obj, tableName)
{
    this.setLastModifiedNode(obj);
    var result = true;
    var table = this.tables[tableName];
...
ExilityPage.prototype.tabClicked = function (obj, tabbedPanelName, tabContainerName)
{
    //we have to display the tabbed panel, and hide the panel that is currently visible.
    var newId = obj.id;
    ...
    debug('Tab action started for panel name ' + tabbedPanelName + ' when user clicked on ' + newId);
    //current active tab and panel names are stored as global variable in window
    ...
    {
        debug('Current active tab panel for table ' + tabbedPanelName + ' could not be found in variables ' + tabVariableName + ' and ' + panelVariableName);
        return;
    }
    ...
    var currentTabLeft = this.doc.getElementById(this.win[tabVariableName] + 'Left');
    if (currentTabLeft)
    {
        currentTabLeft.className = 'passivetablabelLeft';
        currentTabLeft.clickDisabled = null;
    }
    var currentTabRight = this.doc.getElementById(this.win[tabVariableName] + 'Right');
    if (currentTabRight)
    {
...
//twister is cliked on ele, or an internal function wants divName to be hidden/shown
ExilityPage.prototype.twist = function (ele, divName)
{
    ...
    if (div.childNodes.length == 2 && div.childNodes[1].tagName == 'DIV')
    {
        ...
        if (innerDiv.childNodes.length == 2 && innerDiv.childNodes[0].tagName == 'TABLE')
        {
            var tableEle = innerDiv.childNodes[0];
            var tableObj = this.getTable(tableEle.id);
            if (tableObj != null && tableObj.grid && tableObj.grid.length == 1 && this.doc.getElementById(tableEle.id + 'NoData') != null)
    ...
    if (ele)
    {
        ...
        if (imgEle && !imgEle.src)
            imgEle = imgEle.nextSibling; // sometimes, a #text will slip in between
    }
...
ExilityPage.prototype.fillList = function (grids, listServiceOnLoad) // Aug 06 2009 :  Bug ID 631 - suppressDescOnLoad is not working - WeaveIT: Venkat
{
    for (var gridName in this.listServices)
    {
        ...
        debug('got ' + listeners.length + ' listeners for ' + gridName + ' with first one being ' + (listeners[0] && listeners[0].name || 'null'));
        for (var i = 0; i < listeners.length; i++)
            listeners[i].fillList(grid, null, listeners.keyValue, listServiceOnLoad); // Aug 06 2009 :  Bug ID 631 - suppressDescOnLoad is not working - WeaveIT: Venkat
    }
};
...
ExilityPage.prototype.goButtonClicked = function (selectionName)
{
    ...
    if (!ele)
    {
        debug('ERROR : Dom element for buttonDropDown with name = ' + selectionName + ' not found');
        return;
    }
    ...
    {
        // Oct 28 2008 : Incorporating Changes from Tanmay : Aravinda
        message('Please select an action before clicking on GO button', "", "Ok", null, null, null);
        ele.focus();
        return;
    }
...
ExilityPage.prototype.getUnqualifiedNames = function (names)
{
    ...
    for (var i = 0; i < names.length; i++)
    {
        ...
        if (!field)
            debug(uname + ' is not a valid field. getUnqualifiedNames will not work properly');
...
// picks up a row from the grid and copies the columns of this row as values in dc.
// the key is the name of the column, and val is the value to match in the column to select the row
...
//another utility function that goes thru a grid, tries to match a column value with the supplied value
// and returns a grid with the matching rows, (along with the header) Match is assumed when the value
...
var getGridForDescService = function (grid, columnName, valueToMatch)
{
    ...
    if (colIdx == -1)
    {
        debug(columnName + ' is not a column name in the supplied grid that as the header row ' + headerRow);
        return newGrid;
    }
...
var listServiceReturned = function (obj, fieldName, se, listServiceOnLoad)
{
    ...
    if (!fieldName) // service was NOT for a specific field or object, but is a geric one. Let us hand over to fillList
    {
    ...
    if (!field)
    {
        debug('Design error: list service ' + gridName + ' returned successfully for field ' + fieldName + ' but there is no field with this name.');
        return;
    }
    ...
    var grid = null;
    var keyValue = null;
    //grid name should start with list service id, but it could be having _keyValue after that.
    for (var a in grids)
    {
        if (a.indexOf(gridName) != 0)
            continue;  //starting is good enough for us
            
        grid = grids[a];
        if (a.length > gridName.length) //it has keyValue in that
            keyValue = a.substring(gridName.length + 1);
        break;
    }

    if (!grid)
    {
        debug('Error: list service ' + gridName + 'did not come back with a grid name starting with ' + gridName);
        return;
    }
    //is it a list for different key?
    if (obj && obj.listServiceKey && keyValue != obj.listServiceKey)
    {
        debug('Info: list service ' + gridName + ' returned successfully. But the object is waiting for key value ' +  obj.listServiceKey + ' whiel this service was for keyValue ' + keyValue);
        return;
    }
...
{
    debug('reportServiceReturned() for object=' + obj + ' with field name ' + fieldName);
    ...
    {
        var field = P2.getField(fieldName);
        // we have a small issue. What is the name of the grid? as of now, there is no scope of two grids coming in
        // when the request is for a specific object. hence we safely use  the first one.
        for (var a in grids)
        {
            if (a.indexOf(field.reportServiceId) >= 0)
            {
                field.fillReport(grids[a], obj);
                return;
            }
        }
        // Oct 28 2008 : Incorporating Changes from Tanmay : Aravinda
        debug("design error: " + field.name + ' asked for a list service = ' + field.listServiceId + ' but the dc did not have a grid starting with that name', "Error", "Ok", null, null, null);
        return;
    }
...
ExilityPage.prototype.getWindow = function (name)
{
    var win = exilityPageStack.activeWindows[name];
...
{
    //note that we do not call fieldChanged for checkbox group. Hence some functionality is copied.
    ...
    if (!field)
    {
        debug('Design Error: CheckbBoxGroupField : ' + groupName + ' is not rendered properly.');
        return;
    }
    this.setLastModifiedNode(obj);
    this.pageChangedByUser = true;
...
{
    if (setDateFormat(format, separator)) return true;
    // Oct 28 2008 : Incorporating Changes from Tanmay : Aravinda
    message(format + ' is not a valid date format. Specify one of ddmmyy ddmmmyy ddmmyyyy or mmddyy', "Warning", "Ok", null, null, null);
    return false;
};

//a utility function that takes a query string and returns a collection of field/values
var queryFieldsToCollection = function (qryStr)
{
    debug('going to make a collection out of qry string = ' + qryStr);
    var fields = new Object();
    if (!qryStr) return fields;
...
var coverPage = function (coverStyle, imgStyle, imgMainPrgIndStyle)	// Feb 21 2011 : Bug 2119 - Spinner need not appear when there is a message displayed - Exility App : Aravinda
{
    ...
    //NOTE : Bhandi 7-Jan-09 document.documentElement.clientHeight/Width did not work for IE
    ...
    // Feb 21 2011 : Bug 2119 - Spinner need not appear when there is a message displayed - Exility App : Aravinda
    if (imgMainPrgIndStyle)
    {
        imgMainPrgIndStyle.display = '';
    }
    // Feb 21 2011 : Bug 2119 - Spinner need not appear when there is a message displayed - Exility App : Aravinda
    // Oct 21 2008 : Incorporating Changes from Tanmay (End) : Aravinda
    if (coverStyle)
        coverStyle.display = '';
};

var uncoverPage = function (coverStyle, imgMainPrgIndStyle, imgStyle)	// Feb 21 2011 : Bug 2119 - Spinner need not appear when there is a message displayed - Exility App : Aravinda
{
    if (imgStyle)
    ...
    if (imgMainPrgIndStyle)
    {
        imgMainPrgIndStyle.display = 'none';
    }
};
...
ExilityPage.prototype.getComboUrl = function (obj, val, fieldName)
{
    var field = this.fields[fieldName];
    var url;
    if (exilParms.useLocalData || !this.win.location.host || localServices[field.descServiceId])
        url = exilParms.localData + field.descServiceId.replace(/\./g, '/') + '.js';
    else
        url = exilParms.comboServiceName;
    url += '?serviceId=' + field.descServiceId;
    var qry = this.makeQueryString(field.descQueryFields, field.descQueryFieldSources, null, field.validateQueryFields, field.name);
    if (qry == false)
        return null;
    if (qry)
        url += '&' + qry;
...
ExilityPage.prototype.handleGridNav = function (obj, fieldName, e)
{
    ...
    if (keycode == 40 && e.altKey)
    {
        if (field && field.table)
        {
            ...
            {
                curRow = this.doc.getElementById(obj.name.replace(/Radio$/, '')).rowIdx;
            }
            if (curRow < curNbrRows)
            {
                ...
                if (fieldToFocus)
                {
                    if (obj.name && obj.name.match(/.*Radio$/))
                    {
                        if (fieldToFocus.getElementsByTagName("input") && fieldToFocus.getElementsByTagName("input")[0])
                        {
                            fieldToFocus.getElementsByTagName("input")[0].focus();
                        }
                    }
                    else if (obj.className && obj.className == 'checkboxgroup')
                    {
                        if (fieldToFocus.getElementsByTagName("input") && fieldToFocus.getElementsByTagName("input")[0])
                        {
                            fieldToFocus.getElementsByTagName("input")[0].focus();
        ...
        else
        {
            ...
            if (curObjId.match(/Delete__(\d)+$/))
            {
                ...
                var fieldToFocus = this.doc.getElementById(curObjIdParts.join("__"));
                if (fieldToFocus)
                {
                    fieldToFocus.focus();
                }
...
ExilityPage.prototype.comboCall = function (obj, fieldName, val)
{
    ...
    if (this.fillDcFromQueryFields(dc, field.descQueryFields, field.descQueryFieldSources, null, field.validateQueryFields, val, fieldName))
    {
        ...
        for (var i = 0; i < n; i++)
        {
            ...
            {
                if ((dc.values[field.descQueryFields[i]] == "") || (dc.values[field.descQueryFields[i]] != val))
                    dc.addValue(field.descQueryFields[i], val);
            }
        }

        var se = new ServiceEntry(field.descServiceId, dc, false, comboReturned, obj, field.name, ServerAction.NONE, false, false, field.fieldToFocusAfterExecution); //Jun 11 2009 : Bug 507 -  feild Focus - WeaveIT : Aravinda
        se.keyValue = val;
        this.win.serverStub.callService(se);
    }
};
...
ExilityPage.prototype.treeViewExpandCollapseRow = function (objImg, tableName, columnName)
{
    var table = this.tables[tableName];
    table.treeViewExpandCollapseRow(objImg, columnName);
};

ExilityPage.prototype.treeViewSelectDeselectRow = function (objChk, tableName, columnName)
{
    var table = this.tables[tableName];
...
ExilityPage.prototype.treeViewBulkSelectDeselectRow = function (objChk, tableName, columnName)
{
    var table = this.tables[tableName];
...
//this is a temp funcitonality to take care of issue with IE 7 while sliding. If there are any fixed header theader,
...
ExilityPage.prototype.filterTable = function (obj, tableName, columnName)
{
    ...
    var xy = getDocumentXy(td);//modified findXY() to getDocumentXy. Since findXY() is deprecated.
    var ele = document.getElementById(currentActiveWindow.name);
    //alert('current window is at ' +  ele.offsetTop + ' and ' + ele.offsetLeft);
    var x = xy.x + ele.offsetLeft;
    var y = xy.y + ele.offsetTop + 30;
    //alert('I got ' + td.tagName +' element at ' + x + ' and ' + y + ' and towindop is ' + document.body.clientTop);
    if (!window.filterDiv)
...
var LEFT = 37;
var RIGHT = 39;
// Apr 29 2009 : Bug 455 -  List panel Pagination Issue  - WeaveIT (Start) : Venkat
var TAB = 9;
var ZERONUMPAD = 96;
var ONENUMPAD = 97;
var TWONUMPAD = 98;
        ...
        case ENTER:
            {
                ...
                var totalPagesId = field.id.replace(/CurrentPage$/g, 'TotalPages');
                var totalValue = this.doc.getElementById(totalPagesId).innerHTML;
                if (parseInt(curValue) >= 1 && parseInt(curValue) <= parseInt(totalValue))
                {
                    this.tables[field.id.replace(/CurrentPage$/g, '')].paginate('random');
                }
                else
                {
                    message('Invalid Page Number', "Error", "Ok", null, null, null);
                }
            }
...
ExilityPage.prototype.hideOrShowColumns = function (gridName, columnName, displayLists)
{
...
ExilityPage.prototype.focusNext = function (field, currentEle)
{
    ...
    //if this is a field inside a table, we should try and move inside the table
    if (field.table && field.table.focusNext)
    {
        ...
        //no field to move inside this table. We should move to field next to the table
        seqNo = obj.ele.seqNo;
    }
    else
        seqNo = field.seqNo;
    ...
    for (var i = seqNo + 1; i !== seqNo; i++)
    {
        ...
        else
        {
	        ...
	        if (field && field.isEditable)
	        {
		        ...
		        if (!ele)
		        {
		            debug("Error:" + eleName + " has no HTML element. Field is sjipped.");
		        }
    ...
    {
        debug('focus COULD NOT BE shifted to ' + ele.id + '. Error: ' + e + ". Will blur from this field at least.");
    }
    return ele;
};
...
One of the problems in event handling is passing parameters etc..
I allow you to use a method on your object to be called back with event as a sole parameter
BY the way, be a responsible programmr, and do remove the listener once you are done.
**********************************************/
//ele 'click' is what you shoudl supply and not 'onclick'. obj must have a method by fName. method fname will receive (e, ele) where e is event and ele is the dom element that caused the event.
// keep the returned value to be supplied back to me to remove the listener
ExilityPage.prototype.addListener = function (ele, eventNameWithoutOn, obj, fName)
{
    ...
    //debug(idx + ' :' + fName + ' added as on' + eventNameWithoutOn + ' event listener for ele ' + ele.id);
    return idx;
};
...
ExilityPage.prototype.removeListener = function (idx)
{
    ...
    if (!obj)
    {
        debug('design Error: ' + idx + ' is supplied for removing an event listener, but no event listener is available with that idx');
        return;
    }
...
ExilityPage.prototype.moreOptionClicked = function (fieldName, e)
{
    ...
    if (!fn)
    {
        alert('Function ' + (field && field.showMoreFunctoinName) + ' is not defined. Show More option on ' + field.name + ' will not work.');
        return;
    }
    fn(fieldName);
};
